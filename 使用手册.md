...
8. 重启 Obsidian。
9. 进入 `Settings -> Community plugins`。
10. 关闭 `Restricted mode`。
11. 启用 `Science Input Helper`。
...
5. 添加成功后启用 `Science Input Helper`。

### 方法三：从 GitHub Releases 安装

如果仓库已经发布 Release，可以从 Releases 页面下载：
...
1. 点击左侧 ribbon 的烧瓶图标打开快速输入框，或在 `Settings -> Hotkeys` 中给 `Toggle quick input` 绑定快捷键。
